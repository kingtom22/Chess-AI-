...
bool genv::canvas::load(const std::string& file)
{
    buf = SDL_LoadBMP(file.c_str());
    if (!buf)
    {
...
std::string genv::groutput::get_loaded_font_file_name()
{
	return loaded_font_file_name;
}
...
void genv::canvas::draw_text(const std::string& str)
{
    if (font == 0) {
        ...
        if (antialiastext) {
            t = TTF_RenderText_Blended(font, str.c_str(), text_clr);
        } else {
            t = TTF_RenderText_Solid(font, str.c_str(), text_clr);
        }
		if (t != nullptr)
		{
			SDL_Rect offset;
...
void genv::canvas::blitfrom(const genv::canvas &c, short x1, short y1, unsigned short x2, unsigned short y2, short x3, short y3) {
    ...
    if (c.transp) {
        SDL_SetColorKey(c.buf, SDL_SRCCOLORKEY|SDL_RLEACCEL,SDL_MapRGB(c.buf->format, 255, 0 ,0));
    }
